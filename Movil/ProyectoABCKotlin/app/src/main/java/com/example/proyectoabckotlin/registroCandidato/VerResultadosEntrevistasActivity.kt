...
class VerResultadosEntrevistasActivity : AppCompatActivity() {

    private lateinit var binding: ActivityVerResultadosEntrevistasBinding
    private var pruebas: List<Prueba>? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityVerResultadosEntrevistasBinding.inflate(layoutInflater)
        ...
        val token = obtenerTokenDesdeSharedPreferences(this@VerResultadosEntrevistasActivity)
                    ...
                    if (response.isSuccessful) {
                        ...
                        if (entrevistas != null) {
                            ...
                            entrevistas.forEach { entrevista ->
                                ...
                                val cardViewLayout = layoutInflater.inflate(R.layout.cardview_layout_entrevista, null) as CardView
                                ...
                                val cargoTextView: AppCompatTextView = cardViewLayout.findViewById(R.id.cargoTextView)
                                val proyectoTextView: AppCompatTextView = cardViewLayout.findViewById(R.id.proyectoTextView)
                                val estadoTextView: AppCompatTextView = cardViewLayout.findViewById(R.id.estadoTextView)
                                val detallesTextView: AppCompatTextView = cardViewLayout.findViewById(R.id.detallesTextView)

                                // Establecer datos en las vistas
                                nombreEmpresaTextView.text = "${entrevista.empresaNombre ?: ""}"
                                cargoTextView.text = "Cargo al que aplica: ${entrevista.perfilDescripcion ?: ""}"
                                proyectoTextView.text = "Proyecto Asociado: ${entrevista.proyectoNombre ?: ""}"
                                if(entrevista.estado==false) {
                                    estadoTextView.text = "Estado: Pendiente"
                                    detallesTextView.text = "Detalles: "
                                }
                                else {
                                    estadoTextView.text = "Estado : Finalizada"
                                    detallesTextView.text = "Detalles: ${entrevista.detalles ?: ""}"
                                }
        ...
        try{
            ...
            Toast.makeText(
                this@VerResultadosEntrevistasActivity,
                ex.toString(),
                Toast.LENGTH_LONG
            ).show()
        }
